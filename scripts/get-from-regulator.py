...
allowed_attrs = ['TimeInstant', 'Latitud', 'Longitud', 'ActivePower', 'ReactivePower', 'electricPotential', 'electricCurrent']
...
if not attr in allowed_attrs:
    print attr + ' is not an allowed attribute for Regulator'
    print 'allowed ones: ' + str(allowed_attrs)
    exit(1)

p = subprocess.Popen(['./query-regulator.sh'], shell=False, stdout=subprocess.PIPE)
