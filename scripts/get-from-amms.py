...
allowed_attrs = ['TimeInstant', 'Latitud', 'Longitud', 'ActivePower', 'ReactivePower']
...
if not attr in allowed_attrs:
    print attr + ' is not an allowed attribute for AMMS'
    print 'allowed ones: ' + str(allowed_attrs)
    exit(1)

p = subprocess.Popen(['./query-amms.sh'], shell=False, stdout=subprocess.PIPE)
